...
class c_compressor:
    ...
    def _convert_box_vecs(self,lat_params):
        ...
        if self.triclinic:
            ...
            box_vecs = np.zeros((3,3),dtype=float)
            box_vecs[0,0] = _a; box_vecs[1,1] = _b; box_vecs[2,2] = _c
    ...
    def _parse_text_files(self):
        ...
        if self.triclinic:
            ...
            _lat_params = np.zeros((3,2),dtype=float)
    ...
    def _get_meta_data(self):
        ...
        with open(self.lammps_traj_files[0],'r') as _f:
            for ii in range(5):
                _l = _f.readline()
            if len(_l.strip().split()) == 9:
                self.triclinic = True

        if self.triclinic:
            print('\napparently a triclinic simulation box')
        else:
            print('\napparently an orthorhombic simulation box')
