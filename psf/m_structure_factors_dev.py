...
class c_structure_factors:
    ...
    def __init__(self,config,comm,timers):
        ...
        if not self.calc_coherent and not self.calc_incoherent:
            msg = "set alteast one of calc_coherent or calc_incoherent to True"
            crash(msg)

        _num_Q = self.comm.Qpoints.num_Q
        _num_steps = self.comm.traj.num_block_steps

        self.coh_expiQr_sq = np.zeros((_num_Q,_num_steps),dtype=complex)

        # elastic part always calculated
        if self.calc_coherent:
            self.coherent_sq = np.zeros(_num_Q,dtype=float)
        if self.calc_incoherent:
            self.incoherent_sq = 0.0
        ...
        # inelastic part optionally calculated
        if self.calc_sqw:

            self._setup_energies()
            
            if self.calc_coherent:
                self.coherent_sqw = np.zeros((_num_Q,self.num_energy),dtype=float)
            if self.calc_incoherent:
                self.incoherent_sqw = np.zeros((self.num_energy),dtype=float)
    ...
    def calculate_structure_factors(self):
        ...
        if self.calc_coherent:
            self.coherent_sq /= _num_blocks*_num_atoms*_num_steps
        if self.calc_incoherent:
            self.incoherent_sq /= _num_blocks*_num_atoms*_num_steps

        if calc_sqw:
            if self.calc_coherent:
                self.coherent_sqw /= _num_blocks*_num_atoms*_num_steps
            if self.calc_incoherent:
                self.incoherent_sqw /= _num_blocks*_num_atoms*_num_steps
    ...
    def _calculate_on_block(self,block):
        ...
        # do parallel calculation 
            ...
            for _proc in range(_num_procs):
                ...
                proc, coh_expiQr_sq = self.queue.get()
                Q_inds = self.comm.paral.Qpts_on_proc[proc]
                self.coh_expiQr_sq[Q_inds,:] = coh_expiQr_sq[...]
        ...
        self._calc_strufacs(self.coh_expiQr_sq)

        self.timers.stop_timer('calc_on_block')

    # ----------------------------------------------------------------------------------------------

    def _calc_strufacs(self,coh_expiQr_sq):

        """
        take coh_expiQr_sq (rho(Q,t)) and calculate everything from it
        """

        self.timers.start_timer('calc_strufacs',units='s')

        if self.calc_coherent:
            self.coherent_sq += np.abs(np.mean(coh_expiQr_sq,axis=1))**2
        if self.calc_incoherent:
            self.incoherent_sq += np.abs(np.mean(coh_expiQr_sq,axis=1))**2 # dev

        if self.calc_sqw:
            if self.calc_coherent:
                self.coherent_sqw += np.abs(fft(coh_expiQr_sq,axis=1,norm='forward'))**2
            if self.calc_incoherent:
                self.incoherent_sqw += np.abs(fft(coh_expiQr_sq,axis=1,norm='forward'))**2 # dev
    ...
    def _parallel_loop_on_Q(self,proc=0):
        ...
        if self.calc_coherent:
            _coh_expiQr_sq = np.zeros((_nQ,_num_steps),dtype=complex)
        if self.calc_incoherent:
            _inc_expiQr_sq = 
        ...
        for ii in range(_nQ):
            ...
            _coh_expiQr_sq = np.tile(_Q,reps=(_num_steps,_num_atoms,1))
            _coh_expiQr_sq = np.sum(_coh_expiQr_sq*self.comm.traj.pos,axis=2)
            _coh_expiQr_sq = np.exp(1j*_coh_expiQr_sq)*_x_tile
            _coh_expiQr_sq = np.sum(_coh_expiQr_sq,axis=1)

            coh_expiQr_sq[ii,:] = _coh_expiQr_sq

        # put results in queue to be passed to main proc
        self.queue.put([proc,coh_expiQr_sq])
    ...
    def _serial_loop_on_Q(self):
        ...
        #coh_expiQr_sq = np.zeros((_nQ,_num_steps),dtype=complex)
        ...
        for ii in range(_nQ):
            ...
            self.coh_expiQr_sq[_Q_ind,:] = _coh_expiQr_sq
    ...
    def unfold_structure_factors(self):
        ...
        if _symmetry.use_symmetry:
            ...
            if self.calc_sqw:
                if self.calc_coherent:
                    self.coherent_sqw = _symmetry.unfold_onto_full_Q_set(self.coherent_sqw)

            if self.calc_coherent:
                self.coherent_sq = _symmetry.unfold_onto_full_Q_set(self.coherent_sq)
        ...
        if _use_mesh:
            ...
            if self.calc_sqw:
                if self.calc_coherent:
                    self.coherent_sqw = _Qpts.unfold_onto_cartesian_mesh(self.coherent_sqw)

            if self.calc_coherent:
                self.coherent_sq = _Qpts.unfold_onto_cartesian_mesh(self.coherent_sq)
